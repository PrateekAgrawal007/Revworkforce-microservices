...
1. **Shared Database**: All three services connect to the same MySQL database (`revworkforce_db_p3`). This is a pragmatic approach that avoids the complexity of inter-service HTTP calls while still keeping services independently deployable.
...
This creates the `revworkforce_db_p3` database with all tables, departments, designations, leave types, and the default Admin user.
...
| Role | Email | Password |
|------|-------|----------|
| **Employee** | `employee@revworkforce.com` | `Welcome@123` |  (Prateek Agrawal)
| **Manager** | `manager@revworkforce.com` | `Welcome@123` | (Darshan Raval)
